...
const BASE_API = `${API_HOST}:5273/api/WeeklyAmperageReport`;

export const useGetWeeklyAmp = ({ search, page, pageSize }) => {
  const { token } = useSelector((state) => state.auth);

  const getReport = useQuery({
    queryKey: ['weekly', token, search, page, pageSize],
    queryFn: async () => {
...
export const useGetAllWeeklyAmp = () => {
  const { token } = useSelector((state) => state.auth);
...
export const useCreateWeeklyAmp = () => {
  const { token } = useSelector((state) => state.auth);
  ...
  const createReport = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['weekly', token] });
    },
    onError: (e) => {
...
export const useUpdateWeeklyAmp = () => {
  const { token } = useSelector((state) => state.auth);
  const queryClient = useQueryClient();
...
export const useDeleteWeeklyAmp = () => {
  const { token } = useSelector((state) => state.auth);
  const queryClient = useQueryClient();
