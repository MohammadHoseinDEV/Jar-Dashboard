...
import CreateBiWeeklyAmp from '../../components/BiWeeklyAmp/CreateBiWeeklyAmp';
import EditBiWeeklyAmp from '../../components/BiWeeklyAmp/EditBiWeeklyAmp';
import DeleteBiWeeklyAmp from '../../components/BiWeeklyAmp/DeleteBiWeeklyAmp';
import FormBiWeeklyAmp from '../../components/BiWeeklyAmp/FormBiWeeklyAmp';
import Pagination from '../../../../pagination/Pagination';

const ReportActionBiWeeklyAmp = lazy(
  () => import('../../components/BiWeeklyAmp/ReportActionBiWeeklyAmp')
);

function BiWeeklyAmpReport() {
  const { menus: userMenus } = useSelector((s) => s.auth);
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [selectedBiWeeklyAmp, setSelectedBiWeeklyAmp] = useState(null);
  ...
  const perm = useMemo(() => getPerm(userMenus, 'biweekly-amp'), [userMenus]);
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetBiweeklyAmp({ search, page, pageSize });
  const { data: biweeklyAmpAll } = useGetAllBiweeklyAmp();

  const allReport = biweeklyAmp?.data?.totalCount;
  const allConfirmed = biweeklyAmpAll?.data?.filter(
    (d) => d.isSupervisorSigned === true
  ).length;
  const allPending = biweeklyAmpAll?.data?.filter(
    (d) => d.isSupervisorSigned === false && d.isExecutorSigned === true
  ).length;
  const allUnSign = biweeklyAmpAll?.data?.filter(
    (d) => d.isExecutorSigned === false
  ).length;

  const filteredData = useMemo(() => {
    if (filterStatus === 'all') return biweeklyAmp?.data?.items;

    return biweeklyAmpAll?.data?.filter((e) => {
      if (filterStatus === 'confirmed') return e.isSupervisorSigned === true;
      if (filterStatus === 'pending')
        return e.isSupervisorSigned === false && e.isExecutorSigned === true;
      if (filterStatus === 'unSign') return e.isExecutorSigned === false;
      return true;
    });
  }, [filterStatus, biweeklyAmp, biweeklyAmpAll]);

  const countReport = biweeklyAmp?.data?.items?.length;
  const totalPages = biweeklyAmp?.data?.totalPages ?? 1;
  ...
  return (
    <div className="h-screen overflow-hidden rounded-[15px] bg-[#0F090C]/30 text-white">
        ...
        <div className="rounded-[10px] p-[5px] max-md:w-full">
          ...
          <div className="mx-2 mt-1 flex items-center justify-between border-b border-white/30 pb-3">
            ...
            <div className="flex items-center space-x-5 text-white max-md:w-full max-md:justify-between">
              ...
              <p className="space-x-1 text-[20px] max-md:flex max-md:items-center max-md:justify-center max-md:text-[12px]">
                <span className="font-[SamimBold]">گزارش آمپرگیری</span>
                <span className="font-[AvenirLTProMedium]">15</span>
                <span className="font-[SamimBold]">روزه</span>
              </p>
            ...
            <div className="flex items-center space-x-5 pl-5 text-white max-md:hidden">
              ...
              <p className="space-x-1 text-[12px]">
                ...
                <span className="font-[AvenirLTProMedium]">03</span>
              </p>
              <p className="space-x-1 text-[12px]">
                <span className="font-[SamimBold]">کد سند :</span>
                <span className="font-[AvenirLTProMedium]">F1103</span>
              </p>
            </div>
          </div>
          {/* Add Reports & Excel */}
        ...
        <div className="mx-2 hidden max-md:block">
          <div className="my-3 rounded-[10px] border border-[#3a35a0] bg-linear-to-l from-[#201c66] to-[#1d1952] p-2">
            ...
            <div className="flex items-center space-x-2">
              ...
              <p className="text-[15px] font-bold">گزارش آمپر گیری هفتگی </p>
            </div>
            {/* Details Report */}
            <div className="mt-2 grid grid-cols-3 gap-3">
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                ...
                <span className="text-[] font-[AvenirLTProMedium]">F1103</span>
              </p>
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                <span className="text-[11px] font-bold text-[#5e7ef8]">
                  شماره ویرایش
                  ...
                  <tr key={e.id} className="text-center">
                    ...
                    <td className="border-b border-white/20">
                      <ReportActionBiWeeklyAmp
                        ...
                        setSelectedReport={setSelectedBiWeeklyAmp}
                      />
                    </td>
                  </tr>
                ))}
                  ...
                  <div className="mx-2 grid grid-cols-7 gap-2">
                    <button
                      ...
                      className={`col-span-3 mb-2 flex cursor-pointer items-center justify-center rounded-[10px] p-1 font-[Samim] ${
                        (canEdit &&
                          ...
                          !e?.isExecutorSigned) ||
                        (canEdit && isSupervisor && !e?.isSupervisorSigned) ||
                        isSuperAdmin
                    ...
                    <button
                      ...
                      className={`mb-2 flex cursor-pointer items-center justify-center rounded-[10px] p-2 font-[Samim] ${
                        (canDelete && !e?.isSupervisorSigned) || isSuperAdmin
                    ...
                    >
                      <img src={delet} alt="delet" width={20} />
                    </button>
                  </div>
                </div>
              ))}
      ...
      <EditBiWeeklyAmp
        ...
        selectedBiWeeklyAmp={selectedBiWeeklyAmp}
        setSelectedBiWeeklyAmp={setSelectedBiWeeklyAmp}
      />
      <DeleteBiWeeklyAmp
...
export default BiWeeklyAmpReport;
