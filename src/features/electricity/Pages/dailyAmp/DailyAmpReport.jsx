...
import {
  ...
  useGetDailyAmpReport,
  useGetDailyAmpReportAll,
} from '../../Api/dailyAmp/dailyAmp';
import { TbReport } from 'react-icons/tb';
import { HashLoader } from 'react-spinners';
import { toShamsi } from '../../../../Time/date';
import { ReportActionDailyAmp } from '../../components/dailyAmp/ReportsActionDailyAmp';
...
const CreateDailyAmpReport = lazy(
  () => import('../../components/dailyAmp/CreateDailyAmpReport')
);
const EditDailyAmpReport = lazy(
  () => import('../../components/dailyAmp/EditDailyAmpReport')
);
const DeleteDailyAmpReport = lazy(
  () => import('../../components/dailyAmp/DeleteDailyAmpReport')
);
const FormDailyAmpReport = lazy(
  () => import('../../components/dailyAmp/FormDailyAmpReport')
);

function DailyAmpReport() {
  const { menus: userMenus } = useSelector((s) => s.auth);

  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [selectedDailyAmp, setSelectedDailyAmp] = useState(null);
  ...
  const perm = useMemo(
    () => getPerm(userMenus, 'DailyAmp-report'),
    [userMenus]
  );
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetDailyAmpReport({ search, page, pageSize });

  const { data: dailyAmpAll } = useGetDailyAmpReportAll();

  const allReport = dailyAmp?.data?.totalCount;
  const allConfirmed = dailyAmpAll?.data?.filter(
    (d) => d.isSupervisorSigned === true
  ).length;
  const allReject = dailyAmpAll?.data?.filter(
    (d) => d.isSupervisorSigned === false && d.isShiftSupervisorSigned === true
  ).length;
  const allUnSign = dailyAmpAll?.data?.filter(
    (d) => d.isShiftSupervisorSigned === false
  ).length;

  const filteredData = useMemo(() => {
    if (filterStatus === 'all') return dailyAmp?.data?.items;

    return dailyAmpAll?.data?.filter((e) => {
      if (filterStatus === 'confirmed') return e.isSupervisorSigned === true;
      if (filterStatus === 'pending')
        return (
          e.isSupervisorSigned === false && e.isShiftSupervisorSigned !== false
        );
      if (filterStatus === 'unSign') return e.isShiftSupervisorSigned === false;
      return true;
    });
  }, [filterStatus, dailyAmp, dailyAmpAll]);

  const countReport = dailyAmp?.data?.items?.length;

  const totalPages = dailyAmp?.data?.totalPages ?? 1;
  ...
  return (
    <div className="h-screen overflow-hidden rounded-[15px] bg-[#0F090C]/30 text-white">
                  ...
                  <tr key={e.id} className="text-center">
                    ...
                    <td className="border-b border-white/20">
                      <ReportActionDailyAmp
                        ...
                        setSelectedReport={setSelectedDailyAmp}
                      />
                    </td>
                  </tr>
                ))}
      ...
      <EditDailyAmpReport
        ...
        selectedDailyAmp={selectedDailyAmp}
        setSelectedDailyAmp={setSelectedDailyAmp}
      />
      <DeleteDailyAmpReport
