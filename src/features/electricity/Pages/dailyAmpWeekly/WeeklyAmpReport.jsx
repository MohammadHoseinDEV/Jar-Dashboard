...
import {
  useGetAllWeeklyAmp,
  useGetWeeklyAmp,
} from '../../Api/dailyAmpWeekly/WeeklyAmp';
...
import EditWeeklyAmp from '../../components/dailyAmpWeekly/EditWeeklyAmp';
import DeleteWeeklyAmp from '../../components/dailyAmpWeekly/DeleteWeeklyAmp';
import FormWeeklyAmp from '../../components/dailyAmpWeekly/FormWeeklyAmp';

const CreateWeeklyAmp = lazy(
  () => import('../../components/dailyAmpWeekly/CreateWeeklyAmp')
);
const ReportActionWeeklyAmp = lazy(
  () => import('../../components/dailyAmpWeekly/ReportActionWeeklyAmp')
);
const Pagination = lazy(() => import('../../../../pagination/Pagination'));

function WeeklyAmpReport() {
  const { menus: userMenus } = useSelector((s) => s.auth);
  const [search, setSearch] = useState('');
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const [selectedWeeklyAmp, setSelectedWeeklyAmp] = useState(null);
  ...
  const perm = useMemo(() => getPerm(userMenus, 'weekly-amp'), [userMenus]);
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetWeeklyAmp({ search, page, pageSize });
  const { data: weeklyAmpAll } = useGetAllWeeklyAmp();

  const allReport = weeklyAmp?.data?.totalCount;

  const allConfirmed = weeklyAmpAll?.data?.filter(
    (d) => d.isSupervisorSigned === true
  ).length;

  const allPending = weeklyAmpAll?.data?.filter(
    (d) => d.isSupervisorSigned === false && d.isExecutorSigned === true
  ).length;

  const allUnSign = weeklyAmpAll?.data?.filter(
    (d) => d.isExecutorSigned === false
  ).length;

  const filteredData = useMemo(() => {
    if (filterStatus === 'all') return weeklyAmp?.data?.items;

    return weeklyAmpAll?.data?.filter((e) => {
  ...
  }, [filterStatus, weeklyAmp, weeklyAmpAll]);

  const countReport = weeklyAmp?.data?.items?.length;
  const totalPages = weeklyAmp?.data?.totalPages ?? 1;
  ...
  return (
    <div className="h-screen overflow-hidden rounded-[15px] bg-[#0F090C]/30 text-white">
                  ...
                  <tr key={e.id} className="text-center">
                    ...
                    <td className="border-b border-white/20">
                      <ReportActionWeeklyAmp
                        ...
                        setSelectedReport={setSelectedWeeklyAmp}
                      />
                    </td>
                  </tr>
                ))}
      ...
      <EditWeeklyAmp
        ...
        selectedWeeklyAmp={selectedWeeklyAmp}
        setSelectedWeeklyAmp={setSelectedWeeklyAmp}
      />
      <DeleteWeeklyAmp
...
export default WeeklyAmpReport;
