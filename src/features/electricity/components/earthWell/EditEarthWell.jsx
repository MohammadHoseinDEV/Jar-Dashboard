...
import { useupdateEarthWell } from '../../Api/earthWell/earthWell';
...
function EditEarthWell({
  openEditModal,
  setOpenEditModal,
  selectedEarthWell,
  setSelectedEarthWell,
}) {
  ...
  useEffect(() => {
    if (!selectedEarthWell) return;

    setForm({
      id: selectedEarthWell?.id,
      inspectionDate: selectedEarthWell?.inspectionDate,
      personnelName: selectedEarthWell?.personnelName,
      shiftFlags: selectedEarthWell?.shiftFlags,
      items: selectedEarthWell?.items?.length
        ? selectedEarthWell?.items?.map((e) => ({
            id: e?.id,
            wellNumber: e?.wellNumber,
            wellLocation: e?.wellLocation,
  ...
  }, [selectedEarthWell, openEditModal]);
  ...
  const closeHandler = () => {
    ...
    setSelectedEarthWell(null);
  };

  const updateReport = useupdateEarthWell();

  const submitHandler = (e) => {
    e.preventDefault();
    updateReport.mutate(
      { id: selectedEarthWell?.id, form },
      {
        onSuccess: () => {
          closeHandler();
        },
      ...
      >
        <div className="flex shrink-0 items-center justify-between pb-5">
          <h2 className="font-[SamimBold] text-[20px] max-md:text-[15px]">
            ویرایش بازدید ماهیانه چاه های ارت
          </h2>
          <button
            onClick={closeHandler}
          ...
          <div className="grid grid-cols-3 gap-2 max-md:flex max-md:flex-col">
            <label htmlFor="inspectionDate" className="flex flex-col">
              ...
              <DatePicker
                ...
                value={form.inspectionDate ? new Date(form.inspectionDate) : ''}
