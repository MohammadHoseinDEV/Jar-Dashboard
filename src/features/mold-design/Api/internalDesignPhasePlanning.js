...
const BASE_API = `${API_HOST}:5271/api/InternalDesignPhasePlanningForm`;

export const useGetInternalDesignPhasePlanning = ({
  search,
  page,
  pageSize,
}) => {
  ...
  const getReport = useQuery({
    queryKey: ['internal', token, search, page, pageSize],
    queryFn: async () => {
...
export const useGetAllInternalDesignPhasePlanning = () => {
  const { token } = useSelector((state) => state.auth);

  const getReport = useQuery({
    queryKey: ['internal', token],
    queryFn: async () => {
...
export const useCreateInternalDesignPhasePlanning = () => {
  const { token } = useSelector((state) => state.auth);
  ...
  const createReport = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['internal', token] });
    },
    onError: (e) => {
...
export const useUpdateInternalDesignPhasePlanning = () => {
  const { token } = useSelector((state) => state.auth);
  const queryClient = useQueryClient();
...
export const useDeleteInternalDesignPhasePlanning = () => {
  const { token } = useSelector((state) => state.auth);
  const queryclient = useQueryClient();
