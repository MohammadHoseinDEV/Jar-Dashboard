...
function HeaderPageCustomer({
  ...
  return (
    <div className="w-full">
      ...
      <div className="rounded-[10px] p-[5px] max-md:w-full">
        ...
        <div className="mx-2 mt-1 flex items-center justify-between border-b border-white/30 pb-3 max-2xl:pb-1">
          ...
          <div className="flex items-center space-x-5 text-white max-md:w-full max-md:justify-between">
            ...
            <p className="hidden max-md:block max-md:pt-1 max-md:text-[25px]">
              <RiCustomerService2Fill />
            </p>
          </div>
          {/* Details */}
      ...
      {/* data reports */}
      <div className="mx-10 mt-1 grid grid-cols-3 gap-2 space-x-2 max-2xl:mx-5 max-2xl:gap-0 max-md:hidden">
        <div
      ...
      <div className="mx-2 hidden max-md:block">
        <div className="my-3 rounded-[10px] border border-[#3a35a0] bg-linear-to-l from-[#201c66] to-[#1d1952] p-2">
          ...
          <div className="flex items-center justify-between space-x-2">
            <div className="flex items-center space-x-2">
              ...
              <p className="text-[15px] font-bold">مدیرت مشتری ها </p>
            </div>
            <p className="font-[AvenirLTProMedium]">{toShamsi(getdate)}</p>
          </div>
        </div>
        {/* Search input */}
        <div className="relative flex w-full justify-between gap-1">
          ...
          >
            <p className="font-[SamimBold] text-[13px]">افزودن مشتری</p>
        ...
        <div className="mb-2 flex items-center justify-between">
          <div className="flex items-center space-x-1 text-[#6b75a0]">
            <p className="font-[AvenirLTProMedium]">
              {filterStatus === 'all' && allCustomer}
              {filterStatus === 'active' && allCustomerActive}
              {filterStatus === 'inactive' && allcustomerInactive}
            </p>
          ...
          >
            ...
            {openFilterMobile && (
              <div className="absolute top-1 left-11 flex flex-col items-center space-y-2 overflow-auto rounded-[10px] border border-[#3a35a0] bg-linear-to-l from-[#201c66] to-[#1d1952] px-1 py-1 text-nowrap">
                <p
                  onClick={() => {
                    setFilterStatus('all');
                  }}
                  className={`ml-1 rounded-[10px] ${filterStatus === 'all' ? 'bg-gray-200/10 p-2 text-white ' : ''}`}
                >
                  کل مشتری ها
                </p>
                <p
                  onClick={() => setFilterStatus('active')}
                  className={`ml-1 rounded-[10px] ${filterStatus === 'active' ? 'bg-gray-200/10 p-2 text-white ' : ''}`}
                >
                  مشتری های فعال
                </p>
                <p
                  onClick={() => setFilterStatus('inactive')}
                  className={`ml-1 rounded-[10px] ${filterStatus === 'inactive' ? 'bg-gray-200/10 p-2 text-white ' : ''}`}
                >
                  مشتری های غیرفعال
                </p>
              </div>
...
export default HeaderPageCustomer;
