...
function CreateLineChange({ createForm, setCreateForm }) {
  ...
  const getProducts = useMemo(() => {
    const none = { id: '', name: 'انتخاب نشده', code: '' };
          ...
          {form.lineChangeDetails.map((detail, index) => (
            <React.Fragment key={index}>
              ...
              >
                {({ open }) => (
                  <div className="relative">
                    <ComboboxButton className="mt-3 flex w-full items-center justify-between rounded-xl bg-white/10 p-3 font-[Samim] text-[18px] text-white outline-none">
                      <span
                        className={
                          ...
                          filterProducts?.map((p) => (
                            <ComboboxOption
                              ...
                              className={({ active, selected }) =>
                                `cursor-pointer rounded-lg p-3 text-white ${active ? 'bg-black' : ''} ${selected ? 'bg-black' : ''}`
                              }
                            >
                              {p.name} ({p.code})
                            </ComboboxOption>
              ...
              <input
                ...
                readOnly
                className="mt-3 flex w-full items-center justify-between rounded-xl bg-white/10 p-3 font-[Samim] text-[18px] text-white outline-none"
              />
              ...
              <input
                type="number"
                placeholder="میانگین تولید در هر شیفت"
