...
import { useDeleteBachReport } from '../../Api/Bachplant/bachplantApi';

import close from '../../../../assets/images/close.png';

function DeleteBachplantReport({
  openDeleteReport,
  setOpenDeleteReport,
  selectedReports,
}) {
  ...
  const deleteBach = useDeleteBachReport();

  const deleteHandler = () => {
    deleteBach.mutate(selectedReports?.id, {
      onSuccess: () => {
        setOpenDeleteReport(false);
      },
    });
  ...
  return (
    <div
      className={`fixed inset-0 z-50 flex items-center justify-center p-4 transition-opacity duration-300 print:hidden! ${
        openDeleteReport
...
export default DeleteBachplantReport;
