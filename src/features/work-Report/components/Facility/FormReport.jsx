...
function FormReport({
  ...
  const facilityMenu = findMenu(menu?.menus ?? [], 'facility-report');

  const menuId = facilityMenu?.id;
  ...
  // گرفتن عکس امضاء کاربر بر اساس آی دی

  const { data: signHandover } = useGetSignatureId(
    selectedFacility?.shiftHandOverByUserId
  );

  const { data: signReceiver } = useGetSignatureId(
    selectedFacility?.shiftReceiverByUserId
  );

  const { data: signSupervisor } = useGetSignatureId(
    selectedFacility?.supervisorByUserId
  );
  // -------------------------------------------------------

  // مشخص شدن برای اینکه هرکسی گزارش خود را بتواند امضاء کند

  const { data: profile } = useGetProfile();

  const isHandover = selectedFacility?.createdBy === profile?.data?.id;

  const canSignHandover = canSign && isHandover;

  const isSupervisor = profile?.data?.companyRoles?.some(
    (p) =>
      p.roleId === 'a8bc553c-5b99-44cd-8f17-744e2569f4d9' ||
  ...
  useEffect(() => {
    // handover
    if (
      selectedFacility?.isShiftHandOverSigned === true &&
    ...
    // receiver
    if (
      selectedFacility?.isShiftReceiverSigned === true &&
    ...
    // supervisor
    if (
      selectedFacility?.isSupervisorSigned === true &&
    ...
    signReceiver,
    signSupervisor,
    selectedFacility?.isShiftHandOverSigned,
    selectedFacility?.isShiftReceiverSigned,
    selectedFacility?.isSupervisorSigned,
  ]);
  ...
  const rows = useMemo(() => {
    const arr = [...(selectedFacility?.operations ?? [])];
  ...
  return (
    ...
    >
      ...
      >
        <div
          id="form-a4-landscape"
          className="print-section scale-90 border-2 bg-white text-black max-2xl:scale-100"
        ...
        >
              ...
              <tbody className="h-75 border-b">
                {rows.map((f, index) => (
                  <tr key={index} className="border-b text-center">
                    ...
                    <td className="border-l font-[VazirLight] text-[13px]">
                      {f?.operationDescription}
                    </td>
                    <td className="border-l">{f?.requestingUnit}</td>
                    <td className="border-l px-1 font-[AvenirLTProLight] text-[13px]">
                      {normalizeTime(f?.startTime)}
                    </td>
                    <td className="border-l px-1 font-[AvenirLTProLight] text-[13px]">
                      {normalizeTime(f?.endTime)}
                    </td>
                    <td className="font-[VazirLight] text-[13px]">
                      {f?.operationExecutor}
                    </td>
                  </tr>
                ))}
          ...
          <div className="grid h-[90px] grid-cols-2">
            <div className="border-b border-l pt-1 pr-1 text-[15px]">
              <p>قطعات مصرف شده در طول شیفت کاری :</p>
              <div>
                <div className="flex flex-wrap space-x-1">
                  {selectedFacility?.operations
                    ?.map((f, originalIndex) => ({
                      ...f,
            ...
            <div className="space-x-1 border-b pt-1 pr-1 text-[15px]">
              <p>اقلام تحویلی از انبار :</p>
              <div className="flex flex-wrap space-x-1">
                {selectedFacility?.operations
                  ?.filter(
                    (f) =>
