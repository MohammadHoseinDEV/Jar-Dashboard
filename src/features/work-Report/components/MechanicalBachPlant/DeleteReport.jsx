...
import { useDeleteBachReport } from '../../Api/MechanicalBachPlant/mechanicalBachPlant';

function DeleteReport({ openDeleteModal, setOpenDeleteModal, selectedBach }) {
  const closeHandler = () => {
    setOpenDeleteModal(false);
  };

  const deleteReport = useDeleteBachReport();

  const deletehandler = () => {
    deleteReport.mutate(selectedBach?.id, {
      onSuccess: () => {
        setOpenDeleteModal(false);
      },
    });
        ...
        <div>
          <h4 className="pt-5 font-[SamimBold]">
            ...
            <span className="px-2 font-[AvenirLTProMedium]" dir="ltr">
              {selectedBach?.reportNumber}
            </span>
            مطمئن هستید؟
          </h4>
          <div className="mt-6 flex justify-end gap-2" onClick={deletehandler}>
