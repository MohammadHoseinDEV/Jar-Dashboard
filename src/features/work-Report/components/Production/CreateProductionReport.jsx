...
import { useCreateProductionReport } from '../../Api/Production/Production';

function CreateProductionReport({ openCreateModal, setOpenCreateModal }) {
  ...
  const createProductionReport = useCreateProductionReport();
  ...
  const submitHandler = (e) => {
    e.preventDefault();
    createProductionReport.mutate(form, {
      onSuccess: () => {
        setOpenCreateModal(false);
      },
    });
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`relative flex max-h-[90vh] w-[1000px] max-w-[95vw] flex-col gap-4 overflow-hidden rounded-[15px] bg-linear-to-bl from-black to-gray-600 p-6 text-white shadow-2xl transition-all duration-300 ${
          openCreateModal
      ...
      >
        ...
        <form
          ...
          className="no-scrollbar min-h-0 space-y-3 overflow-x-hidden overflow-y-auto max-md:max-h-[70vh]"
        >
