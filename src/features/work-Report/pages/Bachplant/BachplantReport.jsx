...
import { ReportActionBachplant } from '../../components/Bachplant/ReportActionBachplant';

const CreateBachplantReport = lazy(
  () => import('../../components/Bachplant/CreateBachplantReport')
);
const EditBachplantReport = lazy(
  () => import('../../components/Bachplant/EditBachplantReport')
);
const DeleteBachplantReport = lazy(
  () => import('../../components/Bachplant/DeleteBachplantReport')
);
const FormBachplantReport = lazy(
  () => import('../../components/Bachplant/FormBachplantReport')
);

function BachplantReport() {
  ...
  const perm = useMemo(
    () => getPerm(userMenus, 'bachplant-report'),
    [userMenus]
  );
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetBachReport({ search, page, pageSize });
  const { data: bachAll } = useGetAllBachReport();
  ...
  const allReport = bach?.data?.totalCount;
  const allConfirmed = bachAll?.data?.filter(
    (d) => d.isSignedByUnitSupervisor === true
  ).length;
  const allPending = bachAll?.data?.filter(
    (d) => !d.isSignedByUnitSupervisor && d.isSignedByShiftLeader === true
  ).length;
  const allUnSign = bachAll?.data?.filter(
    (d) => !d.isSignedByShiftLeader
  ).length;

  const filteredData = useMemo(() => {
    if (filterStatus === 'all') return bach?.data?.items;

    return bachAll?.data?.filter((e) => {
      if (filterStatus === 'confirmed')
        return e.isSignedByUnitSupervisor === true;
      if (filterStatus === 'pending')
        return (
          e.isSignedByUnitSupervisor === false &&
          e.isSignedByShiftLeader === true
        );
      if (filterStatus === 'unSign') return e.isSignedByShiftLeader === false;
      return true;
    });
  }, [filterStatus, bach, bachAll]);

  const countReport = bach?.data?.items?.length;
  const totalPages = bach?.data?.totalPages ?? 1;

  return (
    <div className="h-screen overflow-hidden rounded-[15px] bg-[#0F090C]/30 text-white">
      <div className="flex h-full flex-col overflow-hidden rounded-[15px] bg-black/70 print:hidden">
        {/* Header */}
          ...
          <div className="mx-2 mt-1 flex items-center justify-between border-b border-white/30 pb-3">
            ...
            <div className="flex items-center space-x-5 pl-5 text-white max-md:hidden">
              ...
              <p className="space-x-1 text-[12px]">
                ...
                <span className="font-[AvenirLTProMedium]">F0504</span>
              </p>
            </div>
          </div>
          {/* Add Reports & Excel */}
          ...
          <div className="my-3 rounded-[10px] border border-[#3a35a0] bg-linear-to-l from-[#201c66] to-[#1d1952] p-2">
            ...
            <div className="mt-2 grid grid-cols-3 gap-3">
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                ...
                <span className="text-[] font-[AvenirLTProMedium]">F0504</span>
              </p>
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                <span className="text-[11px] font-bold text-[#5e7ef8]">
                  شماره ویرایش
              ...
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                ...
                <span className="font-[AvenirLTProMedium]">1404/04/03</span>
              </p>
            </div>
          </div>
          {/* Search input */}
                    ...
                    <td className="border-b border-white/20">
                      <div className="flex items-center justify-center space-x-2">
                        ...
                        <p className="font-[AvenirLTProBook] text-white">
                          {toShamsi(e?.unitSupervisorSignedAt)}
                        </p>
                      </div>
                    </td>
                    <td className="border-b border-white/20">
                      {(() => {
                        if (e?.isSignedByUnitSupervisor === true) {
                          return (
                  ...
                  <div className="mx-1 my-2 flex items-center justify-between border-b border-white/50 px-2 pb-2">
                    <div className="flex items-center space-x-3">
                      ...
                      <p className="flex flex-col space-y-0.5">
                        <span className="font-bold">{e?.operatorName}</span>
                  ...
                  <div className="mx-2 grid grid-cols-7 gap-2">
                    <button
                      ...
                      className={`col-span-3 mb-2 flex cursor-pointer items-center justify-center rounded-[10px] p-1 font-[Samim] ${
                        (canEdit &&
                          ...
                          !e?.isSignedByShiftLeader) ||
                        (canEdit &&
                          isSupervisor &&
                          !e?.isSignedByUnitSupervisor) ||
                        isSuperAdmin
                    ...
                    <button
                      ...
                      className={`mb-2 flex cursor-pointer items-center justify-center rounded-[10px] p-2 font-[Samim] ${
                        (canDelete && !e?.isSignedByUnitSupervisor) ||
                        isSuperAdmin
...
export default BachplantReport;
