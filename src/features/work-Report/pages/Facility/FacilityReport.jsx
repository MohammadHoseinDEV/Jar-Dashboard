...
import {
  useGetAllFacilityReport,
  useGetFacilityReports,
...
const CreateFacilityReports = lazy(
  () => import('../../components/Facility/CreateFacilityReports')
);
const EditFacilityReports = lazy(
  () => import('../../components/Facility/EditFacilityReports')
);
const DeleteFacilityReports = lazy(
  () => import('../../components/Facility/DeleteFacilityReports')
);
const FormReport = lazy(() => import('../../components/Facility/FormReport'));
...
import { ReportActionFacility } from '../../components/Facility/ReportActionFacility';
...
function FacilityReport() {
  ...
  const perm = useMemo(
    () => getPerm(userMenus, 'facility-report'),
    [userMenus]
  );
  ...
  const openCreate = () => {
    if (!canCreate) return toast.error('شما دسترسی ایجاد گزارش ندارید');
  ...
  const allConfirmed = facilityAll?.data?.filter(
    (d) => d.isSupervisorSigned === true
  ).length;
  const allPending = facilityAll?.data?.filter(
    (d) =>
  ...
  const filteredData = useMemo(() => {
    if (filterStatus === 'all') return facility?.data?.items;

    return facilityAll?.data?.filter((e) => {
  ...
  }, [filterStatus, facility, facilityAll]);
  const countReport = facility?.data?.items?.length;
  const totalPages = facility?.data?.totalPages ?? 1;
  ...
  const isSupervisor = profile?.data?.companyRoles?.some(
    (p) => p.roleId === 'a8bc553c-5b99-44cd-8f17-744e2569f4d9'
  );
                  ...
                  <tr key={e.id} className="text-center">
                    ...
                    <td className="border-b border-white/20">
                      <ReportActionFacility
                        ...
                        setSelectedReport={setSelectedFacility}
                      />
                    </td>
                  </tr>
                ))}
