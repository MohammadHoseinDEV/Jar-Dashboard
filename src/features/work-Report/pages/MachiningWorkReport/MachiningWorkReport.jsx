...
import {
  useGetAllMachiningReport,
  useGetMachiningReports,
} from '../../Api/MachiningWorkReport/machiningApi';
import { useGetProfile } from '../../../../hooks/profile/profile';

const CreateReport = lazy(
  () => import('../../components/MachiningWorkReport/CreateReport')
);

const EditMachiningReport = lazy(
  () => import('../../components/MachiningWorkReport/EditMachiningReport')
);
const DeleteMachiningReport = lazy(
  () => import('../../components/MachiningWorkReport/DeleteMachiningReport')
);
const FormReport = lazy(
  () => import('../../components/MachiningWorkReport/FormReport')
);
...
import { ReportActionMachining } from '../../components/MachiningWorkReport/ReportActionMachining';
import { IoTimeOutline } from 'react-icons/io5';
import { HiHashtag } from 'react-icons/hi';

function MachiningWorkReport() {
  ...
  const [openEditReports, setOpenEditReports] = useState(false);
  const [openDeleteReports, setOpenDeleteReports] = useState(false);
  ...
  const perm = useMemo(
    () => getPerm(userMenus, 'machining-report'),
    [userMenus]
  );
  ...
  const openEdit = (m) => {
    if (!canEdit) return toast.warning('شما دسترسی ویرایش گزارش ندارید');

    setOpenEditReports(true);
    setSelectedReports(m);
  };

  const askDelete = (m) => {
    if (!canDelete) return toast.warning('شما دسترسی حذف گزارش را ندارید');
    setOpenDeleteReports(true);
    setSelectedReports(m);
  };
  ...
  const { data: machiningAll } = useGetAllMachiningReport();
  ...
  const isSupervisor = profile?.data?.companyRoles?.some(
    (p) => p.roleId === '7a0bcb84-1e92-4184-91ee-cda1b47a890a'
  );
  const allReport = machining?.data?.totalCount;
  const allConfirmed = machiningAll?.data?.filter(
    (d) => d.isSigned === true
  ).length;
  const allUnSign = machiningAll?.data?.filter(
    (d) => d.isSigned === false
  ).length;

  const filteredData = useMemo(() => {
    if (filterStatus === 'all') return machining?.data?.items;

    return machiningAll?.data?.filter((e) => {
      if (filterStatus === 'confirmed') return e.isSigned === true;

      if (filterStatus === 'unSign') return e.isSigned === false;
      return true;
    });
  }, [filterStatus, machining, machiningAll]);

  const totalPages = machining?.data?.totalPages ?? 1;
  const countReport = machining?.data?.items?.length;

  return (
    <div className="h-screen overflow-hidden rounded-[15px] bg-[#0F090C]/30 text-white">
      {/* <div className="flex h-full flex-col overflow-hidden rounded-[15px] bg-[#0F090C]/40 p-6 text-white">
        <div className="flex shrink-0 items-center justify-between gap-3 rounded-2xl border border-white/50 p-5 text-white">
          <input
            type="text"
            placeholder="جستجو..."
            value={search}
            ...
            className="w-[300px] rounded-xl bg-white/10 p-3 font-[SamimBold] outline-none placeholder:text-white focus:bg-white/70 focus:text-black focus:placeholder:text-black"
          />
          <h1 className="font-[SamimBold] text-xl">
            <span className="relative cursor-default after:absolute after:bottom-0 after:left-1/2 after:h-0.5 after:w-0 after:-translate-x-1/2 after:bg-white/50 after:transition-all after:duration-800 after:ease-out hover:after:w-full">
        ...
        <div className="flex shrink-0 justify-between px-10 pt-5 pb-2">
          <p className="space-x-1">
            <span className="font-[Samim]">کد سند :</span>
            <span className="font-[AvenirLTProMedium]">F1801</span>
          </p>
          <p className="space-x-1">
        ...
        {isLoading ? (
          <div className="flex min-h-0 flex-1 flex-col items-center justify-center space-y-5 py-40 max-2xl:py-10">
            <HashLoader color="#ffffff" size={80} speedMultiplier={1.5} />

            <p className="pt-10 text-[20px]">لطفا منتظر بمانید😎</p>
          </div>
        ) : isError ? (
          <div className="flex min-h-0 flex-1 flex-col items-center justify-center space-y-5 py-40 text-[30px] max-2xl:py-10">
            خطا در دریافت اطلاعات 😟
          </div>
        ) : (
          <div className="no-scrollbar min-h-0 overflow-x-hidden overflow-y-auto rounded-2xl border border-white/70 max-2xl:max-h-[51vh] max-2xl:text-[14px]">
            <table className="w-full border-separate border-spacing-y-2 px-5 py-2">
              <thead>
              ...
              <tbody>
                {machining?.data?.items.map((m, index) => (
                  <tr key={m.id} className="bg-white/5 text-center">
                    <td className="px-3 py-2 font-[AvenirLTProMedium] text-[18px] max-2xl:text-[14px]">
                      {index + 1}
                    </td>
                    <td className="font-[Samim] text-[18px] max-2xl:text-[14px]">
                      {m?.personnelName ?? '--'}
                    </td>
                    <td className="px-3 py-2 font-[AvenirLTProMedium] text-[18px] max-2xl:text-[14px]">
                      {toShamsi(m?.createdAt)}
                    </td>
                    <td className="px-3 py-2 font-[AvenirLTProMedium] text-[18px] max-2xl:text-[14px]">
                      {toShamsi(m?.updatedAt)}
                    </td>
                    <td
                      // onClick={() => {
                      //   navigator.clipboard.writeText(m?.reportNumber);
                      //   toast.info('کپی شد');
                      // }}

                      className="cursor-pointer px-3 py-2 font-[AvenirLTProMedium] text-[18px] max-2xl:text-[14px]"
                    >
                      {m?.reportNumber}
                    </td>
                    <td className="space-x-2 py-2 text-center">
                      <button
                        ...
                        className={`mb-2 cursor-pointer rounded-[10px] p-2 font-[Samim] max-xl:p-1 ${
                          (canEdit && profile?.data?.id === m?.createdBy) ||
                      ...
                      >
                        <img src={deleteIcon} alt="deleteIcon" width={25} />
                      </button>
                      <button
                        data-tooltip-id="my-tooltip"
                        ...
                        className={`mb-2 cursor-pointer rounded-[10px] p-2 font-[Samim] ${'cursor-pointer bg-linear-to-bl from-white/30 to-white/70 transition-all delay-100 duration-150 ease-in-out hover:scale-106'}`}
                      >
          ...
          <div className="mx-2 mt-1 flex items-center justify-between border-b border-white/30 pb-3">
            ...
            <div className="flex items-center space-x-5 text-white max-md:w-full max-md:justify-between">
              ...
              <p className="font-[SamimBold] text-[20px] max-md:flex max-md:items-center max-md:justify-center max-md:text-[12px]">
                گزارش کار روزانه واحد تراشکاری
              </p>
          ...
          <div className="col-span-2 flex items-center space-x-3 py-2">
            <div className="flex items-center space-x-2 rounded-[10px] border border-white/20 bg-[#07090f] px-2 text-white/50">
              ...
              <p
                ...
                className={`my-1 hidden cursor-pointer rounded-[10px] px-4 py-1 transition-all duration-200 ease-out ${
          ...
          <div className="my-3 rounded-[10px] border border-[#3a35a0] bg-linear-to-l from-[#201c66] to-[#1d1952] p-2">
            ...
            <div className="mt-2 grid grid-cols-3 gap-3">
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                ...
                <span className="text-[] font-[AvenirLTProMedium]">F1801</span>
              </p>
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                <span className="text-[11px] font-bold text-[#5e7ef8]">
                  شماره ویرایش
      ...
      <CreateReport
        ...
        closeReportHandler={closeReportHandler}
      />
      <EditMachiningReport
        openEditReports={openEditReports}
        setOpenEditReports={setOpenEditReports}
        selectedReports={selectedReports}
        setSelectedReports={setSelectedReports}
      />

      <DeleteMachiningReport
        openDeleteReports={openDeleteReports}
        setOpenDeleteReports={setOpenDeleteReports}
        selectedReports={selectedReports}
      />
      <FormReport
...
export default MachiningWorkReport;
