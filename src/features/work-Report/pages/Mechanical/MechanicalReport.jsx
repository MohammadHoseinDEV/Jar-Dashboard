...
import {
  useGetAllMechanicalReports,
  useGetMechanicalReports,
} from '../../Api/Mechanical/mechanical';

const CreateMechanicalReports = lazy(
  () => import('../../components/Mechanical/CreateMechanicalReports')
);
const EditMechanicakReports = lazy(
  () => import('../../components/Mechanical/EditMechanicakReports')
);
const DeleteMechanicalReports = lazy(
  () => import('../../components/Mechanical/DeleteMechanicalReports')
);
const FormReport = lazy(() => import('../../components/Mechanical/FormReport'));
import Pagination from '../../../../pagination/Pagination';
import { ReportActionMechanical } from '../../components/Mechanical/ReportActionMechanical';

function MechanicalReport() {
  ...
  const [selectedMechanical, setSelectedMechanical] = useState(null);
  const [filterStatus, setFilterStatus] = useState('all');
  const [openFilterMobile, setOpenFilterMobile] = useState(false);
  // -----------------------------------------------------
  const perm = useMemo(
    () => getPerm(userMenus, 'mechanical-report'),
    [userMenus]
  );
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetMechanicalReports({ search, page, pageSize });

  const { data: mechanicalAll } = useGetAllMechanicalReports();
  ...
  const allReport = mechanical?.data?.totalCount;
  const totalPages = mechanical?.data?.totalPages ?? 1;

  const allConfirmed = mechanicalAll?.data?.filter(
    (d) => d.isSupervisorSigned === true
  ).length;

  const allPending = mechanicalAll?.data?.filter(
    (d) =>
      !d.isSupervisorSigned &&
      d.isShiftHandOverSigned === true &&
      d.isShiftReceiverSigned === true
  ).length;

  const allUnSign = mechanicalAll?.data?.filter(
    (d) =>
      !d.isSupervisorSigned &&
      !d.isShiftHandOverSigned &&
      !d.isShiftReceiverSigned
  ).length;

  const filteredData = useMemo(() => {
    if (filterStatus === 'all') return mechanical?.data?.items;

    return mechanicalAll?.data?.filter((e) => {
      if (filterStatus === 'confirmed') return e.isSupervisorSigned === true;
      if (filterStatus === 'pending')
        return (
          !e.isSupervisorSigned &&
          e.isShiftSupervisorSigned !== true &&
  ...
  }, [filterStatus, mechanical, mechanicalAll]);

  const countReport = mechanical?.data?.items?.length;

  return (
    <div className="z-0 h-screen overflow-hidden rounded-[15px] bg-[#0F090C]/30 text-white">
          ...
          <div className="mx-2 mt-1 flex items-center justify-between border-b border-white/30 pb-3">
            ...
            <div className="flex items-center space-x-5 pl-5 text-white max-md:hidden">
              ...
              <p className="space-x-1 text-[12px]">
                ...
                <span className="font-[AvenirLTProMedium]">F0609</span>
              </p>
            </div>
          </div>
          {/* Add Reports & Excel */}
          ...
          <div className="col-span-2 flex items-center space-x-2 py-2">
            ...
            <div>
              <input
                ...
                className="w-[30vh] rounded-[10px] border border-white/20 bg-[#07090f] py-2.5 placeholder:pr-2 placeholder:text-white/50"
              />
            </div>
          </div>
        </div>
          ...
          <div className="my-3 rounded-[10px] border border-[#3a35a0] bg-linear-to-l from-[#201c66] to-[#1d1952] p-2">
            ...
            <div className="mt-2 grid grid-cols-3 gap-3">
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                ...
                <span className="text-[] font-[AvenirLTProMedium]">F0609</span>
              </p>
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                <span className="text-[11px] font-bold text-[#5e7ef8]">
                  شماره ویرایش
                  ...
                  <tr key={e.id} className="text-center">
                    ...
                    <td className="border-b border-white/20">
                      <ReportActionMechanical
                        ...
                        setSelectedReport={setSelectedMechanical}
                      />
                    </td>
                  </tr>
                ))}
      ...
      <CreateMechanicalReports
        openCreateReport={openCreateReport}
        setOpenCreateReport={setOpenCreateReport}
      />
      <EditMechanicakReports
        openEditReport={openEditReport}
        setOpenEditReport={setOpenEditReport}
        selectedMechanical={selectedMechanical}
        setSelectedMechanical={setSelectedMechanical}
      />
      <DeleteMechanicalReports
