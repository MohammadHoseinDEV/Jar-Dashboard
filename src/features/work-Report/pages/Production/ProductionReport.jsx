...
import {
  useGetAllProductionReport,
  useGetProductionReports,
...
const ProductionFormReport = lazy(
  () => import('../../components/Production/ProductionFormReport')
);
const CreateProductionReport = lazy(
  () => import('../../components/Production/CreateProductionReport')
);
const EditProductionReport = lazy(
  () => import('../../components/Production/EditProductionReport')
);
const DeleteProductionRepor = lazy(
  () => import('../../components/Production/DeleteProductionRepor')
);
import Pagination from '../../../../pagination/Pagination';
import { ReportActionProductionReport } from '../../components/Production/ReportActionProductionReport';
...
function ProductionReport() {
  ...
  // ------------------------------------------------------
  const perm = useMemo(
    () => getPerm(userMenus, 'production-report'),
    [userMenus]
  );
  ...
  const { data: productionAll } = useGetAllProductionReport();

  const allReport = production?.data?.totalCount;

  const allConfirmed = productionAll?.data?.filter(
    (d) => d.isSigned === true
  ).length;

  const allUnSign = productionAll?.data?.filter(
    (d) => d.isSigned === false
  ).length;

  const filteredData = useMemo(() => {
    if (filterStatus === 'all') return production?.data?.items;

    return productionAll?.data?.filter((e) => {
      if (filterStatus === 'confirmed') return e.isSigned === true;
      if (filterStatus === 'unSign') return e.isSigned === false;
      return true;
    });
  }, [filterStatus, production, productionAll]);

  const countReport = production?.data?.items?.length;
  const totalPages = production?.data?.totalPages ?? 1;
  ...
  return (
    <div className="h-screen overflow-hidden rounded-[15px] bg-[#0F090C]/30 text-white">
          ...
          <div className="col-span-2 flex items-center space-x-3 py-2">
            <div className="flex items-center space-x-5 rounded-[10px] border border-white/20 bg-[#07090f] px-2 text-white/50">
              ...
              <p
                ...
                className={`my-1.5 hidden cursor-pointer rounded-[10px] px-4 py-1 transition-all duration-200 ease-out ${
        ...
        <div className="mx-2 hidden max-md:block">
          <div className="my-3 rounded-[10px] border border-[#3a35a0] bg-linear-to-l from-[#201c66] to-[#1d1952] p-2">
            ...
            <div className="mt-2 grid grid-cols-3 gap-3">
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                ...
                <span className="text-[] font-[AvenirLTProMedium]">F0532</span>
              </p>
              <p className="flex flex-col items-center justify-center rounded-[10px] bg-[#343181] py-2">
                <span className="text-[11px] font-bold text-[#5e7ef8]">
                  شماره ویرایش
          ...
          <div className="mb-2 flex items-center justify-between">
            ...
            >
              ...
              {openFilterMobile && (
                <div className="absolute top-1 left-11 flex max-h-40 w-45 flex-col justify-center space-y-3 rounded-[10px] border border-[#3a35a0] bg-linear-to-l from-[#201c66] to-[#1d1952] py-2.5 pr-1.5">
                  <p
                    onClick={() => {
                      setFilterStatus('all');
                    }}
                  ...
                  <p
                    onClick={() => setFilterStatus('pending')}
                    className={`ml-1 hidden rounded-[10px] ${filterStatus === 'pending' ? 'bg-gray-200/10 p-1.5 text-white ' : ''}`}
                  >
                    گزارشات تایید نشده
                  </p>
                  <p
            ...
            <table className="mx-2 w-full border-separate border-spacing-y-0 max-md:hidden">
              <thead className="bg-[#0b0c12] text-white/70">
                <tr>
                  ...
                  <th className="rounded-tl-[10px] border-t border-b border-l border-white/30 pl-1">
                    عملیات
                  </th>
                </tr>
              </thead>
                  ...
                  <tr key={index} className="text-center">
                    ...
                    <td className="border-b border-white/20">
                      <ReportActionProductionReport
                        ...
                        setSelectedReport={setSelectedProduction}
                      />
                    </td>
                  </tr>
                ))}
                ...
                >
                  <div className="mx-1 my-2 flex items-center justify-between border-b border-white/50 px-2 pb-2">
                    <div className="flex items-center space-x-3">
                      ...
                      <p className="flex flex-col space-y-0.5">
                        <span className="font-bold">{p?.personnelName}</span>
                  ...
                  <div className="mx-1 mb-2 grid grid-cols-2 gap-2">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <p className="flex flex-col">
                        <span className="text-white/50">تاریخ امضاء</span>
                        <span className="font-[AvenirLTProMedium]">
                          {toShamsi(p?.signedAt)}
                        </span>
                      </p>
                  ...
                  <div className="mx-2 grid grid-cols-7 gap-2">
                    <button
                      ...
                      className={`col-span-3 mb-2 flex cursor-pointer items-center justify-center rounded-[10px] p-1 font-[Samim] ${
                        (canEdit &&
                          profile?.data?.id === p?.createdBy &&
                          !p?.isSigned) ||
                        (canEdit && isSupervisor && !p?.isSigned) ||
                        isSuperAdmin
                    ...
                    <button
                      ...
                      className={`mb-2 flex cursor-pointer items-center justify-center rounded-[10px] p-2 font-[Samim] ${
                        (canDelete && !p?.isSigned) || isSuperAdmin
      ...
      />
      <EditProductionReport
        openEditModal={openEditModal}
        selectedProduction={selectedProduction}
        setOpenEditModal={setOpenEditModal}
        setSelectedProduction={setSelectedProduction}
      />
      <DeleteProductionRepor
